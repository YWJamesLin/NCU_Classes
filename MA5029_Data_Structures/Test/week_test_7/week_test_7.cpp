...
class Parking_Lot {
  private :
    int max_parking_no, available_parking_no, fee_collected, total_parking_time;
    int served_car_no[2];
    int out_cars[2];
    int lost_cars[2];
  ...
  public :
    Parking_Lot (int parking_no) : max_parking_no (parking_no), available_parking_no (parking_no), fee_collected(0), total_parking_time (0) {
      for (int i = 0; i < 2; ++ i) {
        served_car_no[i] = 0;
        out_cars[i] = 0;
        lost_cars[i] = 0;
    ...
    friend ostream& operator<< (ostream& out, const Parking_Lot<F>& foo) {
      ...
      out << setw (24) << "A" << " 入口" << setw (6) << "B" << " 入口\n\n"
          ...
          << "各入口車子出去數量："
          << setw (10) << foo.out_cars[0] << setw (10) << foo.out_cars[1] << " 輛\n"
          << "各入口平均等候時間："
          ...
          << "各入口失去的車輛數："
          << setw (10) << foo.lost_cars[0] << setw (10) << foo.lost_cars[1] << " 輛\n\n";

      out << "車子進出數量：" << setw (10) << foo.served_car_no[0] + foo.served_car_no[1] << " 輛\n"
          << "停車場總收入：" << setw (10) << foo.fee_collected << " 元\n"
          << "平均停車時間："
          << setw (10) << (foo.total_parking_time / (foo.served_car_no[0] + foo.served_car_no[1])) << " 分\n"
          << "每輛平均收費："
          << setw (10) << (foo.fee_collected / (foo.served_car_no[0] + foo.served_car_no[1])) << " 元\n"
          << "平均等候時間："
          << setw (10) << 60. * (foo.total_waiting_time[0] + foo.total_waiting_time[1]) / (foo.served_car_no[0] + foo.served_car_no[1]) << " 秒\n"
          << "最長等候時間："
          << setw (10) << ((foo.max_waiting_time[0] > foo.max_waiting_time[1]) ? foo.max_waiting_time[0] : foo.max_waiting_time[1]) << " 分\n"
          << "失去的車輛數："
          << setw (10) << foo.lost_cars[0] + foo.lost_cars[1] << " 輛\n";
...
int main () {
  ...
  for (i = 0; i <= 1440; ++ i) {
    ...
    if ( ! waiting_cars0.empty ()) {
      if (i - waiting_cars0.front () > 20) {
        parking_lot.car_lost(0);
        waiting_cars0.pop ();
      }
    }
    if ( ! waiting_cars1.empty ()) {
      if (i - waiting_cars1.front () > 20) {
        parking_lot.car_lost(1);
        waiting_cars1.pop ();
      }
    }
    ...
    if (parking_lot.space_available () && ( ! waiting_cars0.empty () || ! waiting_cars1.empty ())) {
      ...
      } else if ( ! waiting_cars0.empty () && ! waiting_cars1.empty ()) {
        if (waiting_cars1.front () >= waiting_cars0.front ()) {
          w = waiting_cars0.front ();
          waiting_cars0.pop ();
          parking_lot.car_parking (w, i, 0);
        } else {
